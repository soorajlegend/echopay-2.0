  ...
  <div className="mt-[48px] h-auto  h[calc(100vh-100px)] overflow-y-auto">
    <div className="h-[224px] border-2 rounded-[8px] bg-[#D1DCE5] p-4 flex flex-col justify-between">
        <div className=" h-[48px] rounded-full mt-[35px] flex justify-center items-center">
          <Image className="w-[68px] h-[68px] -rotate-[35deg]" src="/mic-01.svg" alt="mic" width={32} height={32} />
          <Image className="w-[68px] h-[68px]" src="/chatting.svg" alt="mic" width={32} height={32} />
        </div>
        <div className="flex justify-between items-center">
          <p className=" text-[18px] text-[#1A1A1A] font-medium">Echo Ai</p>
          <Image className="w-[6px] h-[12px]" src="/right.svg" width={6} height={12} alt="icon" />
        </div>
      </div>
...
const TransactionView = () => (
    ...
    <div className="mt-[48px] justify-between mb-10">
      ...
      <div>
        ...
        <p className="text-[#737373] font-medium mt-[8px]">07 NOV 2024, THURSDAY</p>
      </div>
      <div>
        {transactions.map((transac, index) => <ul className="mt-[10px]">
          <li key={index} className="flex justify-between items-center mt-3">
            <p className="font-medium">{transac.sender}</p>
            <p className={`${transac.status && "text-green-500 font-medium"} text-red-500`}>N{transac.amount}</p>
          </li>
        </ul>)}
      </div>
      </div>
...
const DashboardPage = () => {
  ...
  return (
    <div className="w-full relative h-screen flex flex-col max-w-md mx-auto p-6">
      ...
      {activeView === "home" && (<div className="w-full h-[90px] flex justify-evenly mt-[34px]">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full h-[282px]">
              <div className="flex justify-between border-b relative items-center pb-2 mb-4">
                ...
                <button onClick={() => setShowPopup(false)} className="text-white w-[32px] h-[32px] bg-red-500 flex justify-center items-center  rounded-full absolute -top-8 -right-8 ">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="flex flex-col justify-between h-[80%]">
                <div>
              <p className="mb-4">To ensure the security of your account and enable full access to EchoPay's features, please complete the verification process.</p>
      ...
      <nav className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200">
        ...
        <div className="max-w-md mx-auto flex justify-around items-center h-16">
          ...
          <button onClick={() => setActiveView('home')} className={`flex flex-col items-center ${activeView === 'home' ? 'text-[#003056]' : 'text-gray-500'}`}>
            <Home size={24} />
            <span className="text-xs mt-1">Home</span>
          </button>
          <button onClick={() => setActiveView('transaction')} className={`flex flex-col items-center ${activeView === 'wallet' ? 'text-[#003056]' : 'text-gray-500'}`}>
            <Wallet size={24} />
            <span className="text-xs mt-1">Transaction</span>
          </button>
          <button onClick={() => setActiveView('ai')} className={`flex flex-col items-center ${activeView === 'ai' ? 'text-[#003056]' : 'text-gray-500'}`}>
            <MessageSquare size={24} />
            <span className="text-xs mt-1">Ai</span>
          </button>
          <button onClick={() => setActiveView('more')} className={`flex flex-col items-center ${activeView === 'more' ? 'text-[#003056]' : 'text-gray-500'}`}>
            <Menu size={24} />
